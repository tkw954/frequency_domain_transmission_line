...
T = muto_transmission(omega,r,r,L,c,nu,rho);

%PA/QA with load resistance R_L
Zc=rho*c/(pi*r^2);%characteristic impedance
R_L=1/20*Zc;%(Pa/(m^3/s)) load resistance
...
%% plot
figure(1)
loglog(omega/(pi*c/L),abs(PA_QA_R)/Zc)
ylabel('|P_A/Q_A|/Z_c')
xlabel('\omega/(\pic/L)')
grid on

figure(2)
plot(t/(2*L/c),x_imp/Zc)
grid on
ylabel('P_A/Z_c/(1 m^3/s)')
xlabel('tc/(2L)')
title('Unit impulse response')

figure(3)
plot(t/(2*L/c),x_step/Zc)
grid on
ylabel('P_A/Z_c/(1 m^3/s)')
xlabel('tc/(2L)')
